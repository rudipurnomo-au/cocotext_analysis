...
# Compute detections
def getDetections(groundtruth, evaluation, imgIds = None, annIds = None, detection_threshold = 0.5):
	"""
	A box is a match iff the intersection of union score is >= 0.5.
    Modified to also handle filter using the input annIds
	Params
	------
	...
	for cocoid in imgIds:
		# Also filter by the annotation ID        
		gt_bboxes = set(groundtruth.imgToAnns[cocoid]).intersection(annIds) \
